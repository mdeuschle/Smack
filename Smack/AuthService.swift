...
class AuthService {
    ...
    func createUser(name: String, email: String, avatarName: String, avatarColor: String, completion: @escaping Completion) {
        ...
        Alamofire.request(URLString.urlUserAdd.rawValue, method: .post, parameters: body, encoding: JSONEncoding.default, headers: bearerHeader).responseJSON { (response) in
            if response.result.error == nil {
                guard let data = response.data else {
                    return
                }
    ...
    func findUserByEmail(completion: @escaping Completion) {
        Alamofire.request(URLString.urlUserByEmil.rawValue + email, method: .get, parameters: nil, encoding: JSONEncoding.default, headers: bearerHeader).responseJSON { (response) in
            if response.result.error == nil {
                guard let data = response.data else {
                    return
                }
